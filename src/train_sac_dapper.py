...
class StressDetectionEnv(gym.Env):
    ...
    def reset(self, seed = None, options = None):
        ...
        if len(self.env_data) < self.config.env_length - 2 or self.current_pid in validation_subjects:
            return self.reset(seed=seed)

        valence = self.env_data["Valence"].iloc[0]
        arousal = self.env_data["Arousal"].iloc[0]
        self.label = np.array([valence, arousal], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    cfg = Config()
    env = StressDetectionEnv(config = cfg, seed = 1)
    name = f"{timestamp}_{cfg.rew_mode}_{cfg.rew_func}"
    ...
    model = SAC("MlpPolicy", env, policy_kwargs=policy_kwargs, verbose=1, learning_rate=0.0003, batch_size=256, buffer_size=100000, tensorboard_log=f"./run/{name}")
