...
@torch.compile
class NormwearCWTMLP(nn.Module):
    ...
    for opt in ["NormWear"]: #["NormWear"]:
        ...
        for sid in SUBJECT_IDS:
            ...
            if opt == "NormWear_Embed":
                ...
                input_dim = embeding_length
            elif opt == "NormWear_CWT":
                cwt_data_file = "/diniuvol/jonah/Eric/data/WESAD/WESAD_CWT_NormWear_Data_6s_6s.parquet"
                hf_dataset = hgDataset.from_parquet(cwt_data_file)
                hf_dataset.set_format(type="torch", columns=["Normwear_CWT", "Label"])

                train_dataset = hf_dataset.filter(lambda row: row["PID"] in set(train_ids))
                val_dataset = hf_dataset.filter(lambda row: row["PID"] in set(val_ids))
                test_dataset = hf_dataset.filter(lambda row: row["PID"] in set(test_ids))
                ...
                test_dataset = test_dataset.map(lambda row: {"Label": 0 if row["Label"] > 1 else row["Label"]})
            ...
            if opt == "NormWear":
                ...
                model = NormwearCWTMLP(embedding_length=input_dim, num_class=num_class, normwear_weight_path="./src/NormWear/normwear_pretrain_ckpt.pth").to(device)              
            ...
            for epoch in range(epochs):
                ...
                model.train()
                met = train_one_epoch(model, criterion, train_loader, optimizer, device, epoch, scaler, args = args, log_writer = writer)
                
                # 4. Validation & Metrics Phase
                model.eval()

                val_metrics = evaluate(None, val_loader, model, device, criterion, True)
                test_metrics  = evaluate(None, test_loader, model, device, criterion, True)
                
                # Log metrics to TensorBoard
                writer.add_scalar(f'{sid}/Validation/Loss', val_metrics['loss'], epoch)
                writer.add_scalar(f'{sid}/Validation/F1', val_metrics['f1_macro'], epoch)
                writer.add_scalar(f'{sid}/Validation/WeightedF1', val_metrics['f1_weighted'], epoch)
                writer.add_scalar(f'{sid}/Validation/ACC', val_metrics['accuracy'], epoch)
                writer.add_scalar(f'{sid}/Test/Loss', test_metrics['loss'], epoch)
                writer.add_scalar(f'{sid}/Test/F1', test_metrics['f1_macro'], epoch)
                writer.add_scalar(f'{sid}/Test/WeightedF1', test_metrics['f1_weighted'], epoch)
                writer.add_scalar(f'{sid}/Test/ACC', test_metrics['accuracy'], epoch)
